...
environment variable to `1`) to run the tests in a program in a number order.
This helps to reveal bad dependencies between tests.

By default, GoogleTest uses a number seed calculated from the current time.
Therefore you'll get a different order every time. The console output includes
the number seed value, such that you can reproduce an order-related test failure
later. To specify the number seed explicitly, use the `--gtest_random_seed=SEED`
...
number seed and re-shuffle the tests in each iteration.
